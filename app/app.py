...
db = client.flask_db
razas_de_gatos = db.razas_de_gatos
...
def api_all():
    ...
    '''
    for raza in razas_de_gatos.find():
...
def nueva_raza():
    if request.method == 'POST':
        ...
        max_id = max((raza['id'] for raza in razas_de_gatos.find()), default=0) + 1
        raza_de_gato['id'] = max_id
        razas_de_gatos.insert_one(raza_de_gato)
        return '''
...
def eliminar_raza(id):
    if request.method == 'POST':
        razas_de_gatos.delete_one({"id": id})
        return '''
